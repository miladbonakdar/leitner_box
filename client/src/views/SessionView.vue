<template>
    <div class="p-sm-5">
      ...
      "
          ...
          >
                ...
                <span class="text-muted font-weight-bolder">
                  ...
                  data.item.createdAt | moment("YYYY-MM-DD")
                    ...
                    <div class="col-sm-12 col-md-6">
                        <h4 class="text-muted">Random suggestion
                            <b-icon @click="loadRandomSuggestions" v-b-popover.hover.top="'Reload random cards'"
                                    class="pointer mx-2" scale="1.2" variant="success" icon="arrow-clockwise"></b-icon>
                        </h4>
                        <b-table
                                small
                                hover
                ...
                <span class="text-muted font-weight-bolder">
                  ...
                  data.item.createdAt | moment("YYYY-MM-DD")
                    ...
                    <div class="col-sm-12">
                        ...
                        <hr/>
                        <b-button-group class="mr-2 mb-2" v-for="card in zeroList" v-bind:key="card.id">
                            <b-button v-b-popover.hover.top="card.back" @click="playCard(card.front, 0.8)"
                                      variant="info">
                                {{card.front}}
                                <b-icon
                                        class="pointer mx-1"
        ...
        <div v-if="info.session.isOpen && (info.session.lastSlot !== 0 || (info.session.lastSlot === 0 && zeroListFinalized))"
             class="col-sm-12 mt-3">
            <b-form-group class="mb-5">
                <label class="text-muted h6">Cards progress : </label>
                <b-progress :max="info.cards.length" height="1.3rem" variant="warning" animated>
                    <b-progress-bar :value="cardProgressValue">
            <span
            >Card:
              <strong>{{ cardProgressValue }} / {{ info.cards.length }}</strong></span
            >
                    </b-progress-bar>
                </b-progress>
            </b-form-group>
            <div class="w-100">
                ...
                >
                    ...
                    <hr/>
                    <b-button :active="!stepFinished" class="float-left" variant="danger" @click="moveNext(false)">
                        <b-icon icon="x"></b-icon>
                        NO
                    </b-button>
                    <b-button :active="!stepFinished" class="float-right" variant="success" @click="moveNext(true)">
...
<script>
    ...
    import {
        ...
        createNewSession,
        getSuggestions,
        getFavorites, getRandomSuggestions
    } from "../gate";
    ...
    export default {
        ...
        computed: {
            ...
            progressValue() {
                let step = 6
                if (this.info.session.lastSlot === 15)
                    step = 5
                if (this.info.session.lastSlot === 8)
                    step = 4
                if (this.info.session.lastSlot === 4)
                    step = 3
                if (this.info.session.lastSlot === 2)
                    step = 2
                if (this.info.session.lastSlot === 1)
                    step = 1
            ...
            cardProgressValue() {
                return this.answers.correctAnswers.length + this.answers.wrongAnswers.length + 1;
            },
            stepFinished() {
                return this.answers.correctAnswers.length + this.answers.wrongAnswers.length === this.info.cards.length
            }
        },
        methods: {
            ...mapMutations({
            ...
            checkForGameState() {
                if (this.info.session.isOpen) {
                    ...
                    } else if (this.zeroListFinalized)
            ...
            keyPressed(e) {
                ...
                if (this.info.session.isOpen) {
                    ...
                    if (this.info.session.lastSlot === 0 && !this.zeroListFinalized && e.which === 13)
                        this.confirmCards()
                    if (this.info.session.lastSlot === 0 && this.zeroListFinalized && (e.which === 13 || e.which === 89))
                        this.moveNext(true)
                    if (this.info.session.lastSlot !== 0 && (e.which === 13 || e.which === 89))
                        this.moveNext(true)
